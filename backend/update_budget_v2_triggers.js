...
async function updateBudgetV2Triggers() {
    try {
        console.log("Updating triggers for BUDGET_V2...");
        ...
        await conn.query(`
            ...
            BEGIN
                ...
                VALUES ('BUDGET_V2', 'INSERT', NEW.Budget_ID, NEW.Budget_ID, NEW.User_ID, CONCAT('Created Budget: ', NEW.Budget_Name, ' (₹', NEW.Total_Budget_Amount, ')'), NOW());
            END
        `);
        ...
        await conn.query(`
            ...
            BEGIN
                ...
                IF NEW.Status = 'Deleted' AND OLD.Status != 'Deleted' THEN
                    INSERT INTO AUDIT_LOG (Table_Name, Action_Type, Record_ID, Budget_ID, Changed_By_User_ID, Description, Timestamp)
                    VALUES ('BUDGET_V2', 'DELETE', NEW.Budget_ID, NEW.Budget_ID, NEW.User_ID, CONCAT('Deleted Budget: ', NEW.Budget_Name), NOW());
                
                -- standard update
                ELSEIF OLD.Total_Budget_Amount != NEW.Total_Budget_Amount THEN
                    INSERT INTO AUDIT_LOG (Table_Name, Action_Type, Record_ID, Budget_ID, Changed_By_User_ID, Description, Timestamp)
                    VALUES ('BUDGET_V2', 'UPDATE', NEW.Budget_ID, NEW.Budget_ID, NEW.User_ID, CONCAT('Updated Budget Amount: ', OLD.Total_Budget_Amount, ' -> ', NEW.Total_Budget_Amount), NOW());
                
                ELSEIF OLD.Status != NEW.Status THEN
                    INSERT INTO AUDIT_LOG (Table_Name, Action_Type, Record_ID, Budget_ID, Changed_By_User_ID, Description, Timestamp)
                    VALUES ('BUDGET_V2', 'UPDATE', NEW.Budget_ID, NEW.Budget_ID, NEW.User_ID, CONCAT('Budget Status: ', OLD.Status, ' -> ', NEW.Status), NOW());
                END IF;
            END
        `);
        ...
        conn.release();
        console.log("BUDGET_V2 triggers updated successfully!");
        process.exit(0);
